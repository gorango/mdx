...
# dependencies = ["polars"]
# ///
"""Fetch BTC.D (BTC dominance) from CoinGecko.
...
def compute_btc(btc: dict[int, float], others: dict[str, dict[int, float]]) -> list[tuple[int, float]]:
    n = len(others) + 1
    tss = sorted(set(btc.keys()) | {ts for c in others.values() for ts in c})
    out = []
    for ts in tss:
        b = btc.get(ts, 0)
        vals = {k: v.get(ts, 0) for k, v in others.items()}
        p = sum(1 for v in vals.values() if v > 0) + (1 if b > 0 else 0)
        total = sum(vals.values()) + b
        if total > 0 and b > 0 and p >= n * 0.8:
            out.append((ts, (b / total) * 100.0))
    return out


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--output", default="data/btcd")
    ...
    btc_entry = next((c for c in coin_list if c["symbol"].upper() == "BTC"), None)
    if not btc_entry:
        print("ERROR: BTC not in top coins")
        sys.exit(1)

    coins_to_fetch = [
        c for c in coin_list
        if c["id"] == btc_entry["id"] or c["symbol"].upper() not in EXCLUDED
    ]
    ...
    # ── Compute BTC.D ──
    btc_data = dict(all_data.get(btc_entry["id"], []))
    others = {k: dict(v) for k, v in all_data.items() if k != btc_entry["id"]}
    pts = compute_btc(btc_data, others)
    ...
    if args.backfill:
        print("Backfill: replacing existing data entirely")
    elif existing is not None and len(existing) > 0:
